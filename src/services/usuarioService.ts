...
export class UsuarioService {
	...
	public url = "http://192.168.1.99/bigApp/bigApp/web/api/usuario";
	constructor(private _http: Http){
	}

	 loginAction(json:any){
		return this._http.post("http://192.168.1.99/bigApp/bigApp/web/oauth/v2/token",json,{headers: new Headers({'Content-Type':'application/json'})})
		.map(res => res.json());
	}
